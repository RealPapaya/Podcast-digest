  ...
  "stocks": [
    {
      "name": "台積電",
      "ticker": "2330",
      "exchange": "台股 | 上櫃 | 興櫃 | 美股 | 日股 | 港股 | ETF（擇一）",
      "sector": "半導體",
      "stance": "看多 | 觀望 | 看空",
      "risk": "低 | 中 | 高",
...
注意：
- exchange 欄位只能填以下其中一個值：台股、上櫃、興櫃、美股、日股、港股、ETF
  • 台股 = 台灣證交所上市（4位數字代號，如 2330）
  • 上櫃 = 台灣櫃買中心（如 6488）
  • 興櫃 = 台灣興櫃市場（如 6872）
  • 美股 = 美國股市（英文代號，如 NVDA、AAPL）
  • 日股 = 日本東京交易所（4位數字代號，如 7203）
  • 港股 = 香港交易所（數字代號，如 700、9988）
  • ETF = 各市場 ETF（如 0050、VOO、00878）；ETF 的 exchange 另填實際掛牌市場
- news 抓 3–6 條最重要新聞
...
def analyze_audio_gemini(audio_path: Path, episode: dict) -> Optional[dict]:
    ...
    if result:
        # Always override with authoritative episode info from RSS
        result["ep_number"] = episode.get("ep_number", result.get("ep_number", ""))
        result["date"] = episode.get("date", result.get("date", ""))
